...
mbbARsim <- function(n, M, K, b){
  ...
  for (j in 1:M){
    ...
    data <- genARirr(n = n, parm = c(-0.1, 0.6))
    ...
    for (i in 1:K) {
      sample <- mbbsample(data = data$X, b = b)
      xbar[i] <- mean(sample)
      med[i] <- median(sample)
    }
  ...
  results <- data.frame(coverage_mean = sum(mean_res) / M,
                        ...
                        MSE_mean = (1 / M) * sum(((sqrt(nj) * mean_sd) - (sqrt(n) * 0.0995))^2),
                        MSE_median = (1 / M) * sum(((sqrt(nj) * median_sd) - (sqrt(n) * 0.1128))^2),
